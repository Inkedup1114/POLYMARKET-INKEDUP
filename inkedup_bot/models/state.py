...
class OrderState(BaseModel):
    ...
    transaction_hashes: list[str] = Field(
        default_factory=list, description="Related transaction hashes"
    )
    ...
    def update_fill(self, fill_size: Decimal, fill_price: Decimal) -> None:
        ...
        if self.average_fill_price is None:
            ...
            ) * self.average_fill_price + fill_size * fill_price
...
class PositionState(BaseModel):
    ...
    def add_fill(self, fill_size: Decimal, fill_price: Decimal, is_buy: bool) -> None:
        ...
        if new_side != PositionSide.FLAT and new_side != current_side:
            ...
            self.size = fill_size - (
                self.size if current_side != PositionSide.FLAT else 0
            )

        self.unrealized_pnl = self.calculate_unrealized_pnl()
        self.last_update = datetime.utcnow()

    def _calculate_new_side(
        self, current_side: PositionSide, fill_size: Decimal, is_buy: bool
    ) -> PositionSide:
...
class MarketDataState(BaseModel):
    ...
    best_bid: Decimal | None = Field(None, description="Best bid price")
    best_ask: Decimal | None = Field(None, description="Best ask price")
    last_trade_price: Decimal | None = Field(None, description="Last trade price")
    ...
    spread: Decimal | None = Field(None, description="Bid-ask spread")
    mid_price: Decimal | None = Field(None, description="Mid-market price")

    @validator("best_bid", "best_ask", "last_trade_price", "spread", "mid_price")
    def validate_price_range(cls, v: Decimal | None) -> Decimal | None:
    ...
    def update_from_book(
        self, bids: list[dict[str, Decimal]], asks: list[dict[str, Decimal]]
    ) -> None:
        """
    ...
    def get_liquidity_metrics(self) -> dict[str, Decimal]:
        ...
        return {
            ...
            "spread": spread,
            "spread_bps": (
                (spread / mid_price * 10000) if mid_price > 0 else Decimal("0")
            ),
        }
...
class TradingState(BaseModel):
    ...
    orders: dict[str, OrderState] = Field(
        default_factory=dict, description="Order ID to order state mapping"
    )
    positions: dict[str, PositionState] = Field(
        default_factory=dict, description="Market to position state mapping"
    )
    market_data: dict[str, MarketDataState] = Field(
        default_factory=dict, description="Market to market data mapping"
    )
    ...
    def get_position(self, market: str) -> PositionState | None:
    ...
    def get_market_data(self, market: str) -> MarketDataState | None:
    ...
    def get_open_positions(self) -> list[PositionState]:
        ...
        return [
            ...
            if pos.side != PositionSide.FLAT and pos.size > 0
        ]
...
class StateManager:
    ...
    def create_initial_state(owner: str) -> TradingState:
        ...
        return TradingState(owner=owner, last_update=datetime.utcnow())
