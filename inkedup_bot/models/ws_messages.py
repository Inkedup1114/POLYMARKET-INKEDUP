...
from typing import List, Optional, Dict, Any, Union, cast, Type, Literal
...
class TradeMessage(BaseModel):
    ...
    maker_orders: List[Dict[str, Any]] = Field(..., alias="makerOrders", description="Array of maker orders")
    ...
    def validate_maker_orders(cls, v: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for order in v:
            ...
            if not all(field in order for field in required_fields):
                raise ValueError(f"Maker order missing required fields: {required_fields}")
...
class OrderMessage(BaseModel):
    ...
    @classmethod
    def parse_decimal(cls, v: Union[str, Decimal]) -> Decimal:
...
def parse_websocket_message(data: Dict[str, Any]) -> WebSocketMessage:
    ...
    try:
        ...
        raise ValueError(f"Failed to parse {message_type} message: {str(e)}")
