...
class RealTimeExposureTracker:
    ...
    def __init__(
        ...
        self._last_snapshot: ExposureSnapshot | None = None
        ...
        async with self._reconciliation_lock:
            ...
            for token_id, actual_position in actual_positions.items():
                ...
                if calculated_position > 0:
                    drift_ratio = (
                        abs(actual_notional - calculated_position) / calculated_position
                    )
    ...
    def add_snapshot_callback(
        self, callback: Callable[[ExposureSnapshot], None]
    ) -> None:
            ...
            try:
                ...
                try:
                    ...
                    first_update = await asyncio.wait_for(
                        self._position_updates.get(), timeout=1.0
                    )
    ...
    async def _periodic_reconciliation(self) -> None:
        ...
        while self._running:
            try:
                ...
                if (
                    current_time - self._last_reconciliation
    ...
    async def _calculate_position_pnl(
        self, token_id: str, position: PositionUpdate
    ) -> float:
