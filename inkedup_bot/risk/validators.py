...
class ValidationContext:
    ...
    def __init__(self, config: Any, market_data: dict | None = None):
    ...
    async def validate(
        ...
        try:
            ...
            adjusted_limit = limit * (
                1 + (volatility_factor - 1) * self.adjustment_factor
            )
...
def create_market_condition_validators(
    market_data_provider=None,
) -> dict[str, BaseValidator]:
    """Create market condition validators with market data provider."""
    return {
        "volatility_limit": create_volatility_validator(
            market_data_provider=market_data_provider
        ),
        "liquidity_position": create_liquidity_validator(
            market_data_provider=market_data_provider
        ),
        "market_status": create_status_validator(
            market_data_provider=market_data_provider
        ),
        "correlation_risk": create_correlation_validator(
            market_data_provider=market_data_provider
        ),
    }
