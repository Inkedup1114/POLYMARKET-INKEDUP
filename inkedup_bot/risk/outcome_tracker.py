...
class OutcomeExposureTracker:
    ...
    def __init__(
        ...
        self._last_snapshot: OutcomeExposureSnapshot | None = None
        self._running = False
        ...
        self._max_outcome_exposure = (
            max_outcome_exposure if max_outcome_exposure is not None else 10000.0
        )
    ...
    async def get_outcome_exposure(self, outcome_id: str) -> OutcomeExposure | None:
    ...
    def add_snapshot_callback(
        self, callback: Callable[[OutcomeExposureSnapshot], None]
    ) -> None:
            ...
            try:
                ...
                try:
                    first_update = await asyncio.wait_for(
                        self._outcome_updates.get(), timeout=1.0
                    )
    ...
    async def _process_batch_outcome_updates(
    ...
    async def _update_risk_score(self, outcome_id: str) -> None:
        ...
        if outcome_id in self._correlations:
            high_corr_count = sum(
                1
                for corr in self._correlations[outcome_id].values()
        ...
        async with self._correlation_lock:
            ...
            for i, outcome_a in enumerate(outcomes):
                for outcome_b in outcomes[i + 1 :]:
                    ...
                    price_ratio_a = exposure_a.current_price / max(
                        exposure_a.average_price, 0.001
                    )
                    price_ratio_b = exposure_b.current_price / max(
                        exposure_b.average_price, 0.001
                    )

                    # Simple correlation based on direction
                    correlation = (
                        1.0 if (price_ratio_a - 1) * (price_ratio_b - 1) > 0 else -1.0
                    )
                    ...
                    self._correlations[outcome_a][outcome_b] = OutcomeCorrelation(
                        ...
                        * exposure_a.notional_value
                        * exposure_b.notional_value,
                    )
    ...
    async def get_exposure_by_market(
    ...
    async def get_exposure_heatmap(self) -> dict[str, dict[str, float]]:
        ...
        for _outcome_id, exposure in self._outcome_exposures.items():
            heatmap[exposure.market_slug][
